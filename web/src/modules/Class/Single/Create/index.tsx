...
class SingleClass extends Component<propsType, S> {
	...
	uniqueSubjects = (): Set<string> => {
		...
		Object.values(this.props.classes).forEach((cl) => {
			Object.keys(cl.subjects).forEach((subj) => subjects.add(subj))
		})
	...
	onSave = () => {
		const compulsoryFields = checkCompulsoryFields(this.state.class, [['name']])
		...
		setTimeout(
			() => this.setState({ redirect: this.id() === undefined, banner: { active: false } }),
			1500
		)
	}
	...
	removeClass = (mis_class: AugmentedMISClass) => {
		...
		Object.values(this.props.students).forEach((student) =>
			Object.keys(mis_class.sections).forEach((section) => {
			...
			<div className="single-class">
				...
				<div className="form">
					...
					<div className="row">
						...
						<input
							type="number"
							{...this.former.super_handle(['classYear'])}
					...
					<div className="subject row">
						<input
							list="subjects"
							{...this.former.super_handle(['new_subject'])}
					...
					{Object.values(this.state.class.sections).length === 1 ? (
						false
					) : (
						<div className="divider">Sections</div>
					)}
					{Object.entries(this.state.class.sections).map(([id, section], i, arr) => {
						return (
							<div className={arr.length === 1 ? '' : 'section'} key={id}>
								{arr.length === 1 ? (
									false
								) : (
									<div className="row">
										...
										<input
											type="text"
											{...this.former.super_handle(['sections', id, 'name'])}
								...
								{(this.props.ilmxUser ? !this.isNew() : true) && (
									<>
										<div className="row">
											<label>
												{arr.length === 1 ? 'Teacher' : 'Section Teacher'}
											</label>
											<select
								...
								{arr.length === 1 ? (
									...
									<div className="button red" onClick={this.removeSection(id)}>
										Delete Section
									</div>
								)}
							</div>
					...
					<div className="button green" onClick={this.addSection}>
						Add Another Section
					</div>
					<DefaultFeeSettings
					...
					<div className="save-delete">
						{!this.isNew() ? (
							<div
								className="button red"
								onClick={() => this.removeClass(this.state.class)}>
								Delete
							</div>
						) : (
							false
...
export default connect(
	...
	(dispatch: Function) => ({
		...
		addStudent: (section_id: string, student: MISStudent) =>
			dispatch(addStudentToSection(section_id, student)),
		...
		removeSection: (classId: string, sectionId: string) =>
			dispatch(deleteSection(classId, sectionId)),
		removeSubject: (classId: string, subject: string) =>
			dispatch(deleteSubject(classId, subject)),
