...
export const LedgerPage: React.SFC<LedgerPageProp> = ({
	relevant_payments,
	students,
	settings,
	curr_class,
	...
	return (
		<div className="payment-history">
			{Object.entries(relevant_payments).map(
				([s_id, { filteredPayments, owed, totalOwed }]) => {
					const curr_student = students[s_id]
					return (
						<div key={s_id} className="payment-history section print-page">
							...
							<div className="row info">
								<label style={{ color: owed <= 0 ? '#5ECDB9' : '#FC6171' }}>
									{' '}
									{owed <= 0 ? 'Total Advance:' : 'Total Pending:'}
								</label>
								<div
									style={{
										color: owed <= 0 ? '#5ECDB9' : '#FC6171',
									}}>{` Rs. ${numberWithCommas(Math.abs(totalOwed))}`}</div>
							</div>
							...
							<div className="mis-table row last">
								<label style={{ color: owed <= 0 ? '#5ECDB9' : '#FC6171' }}>
									<b>{owed <= 0 ? 'Advance:' : 'Pending:'}</b>
								</label>
								<div style={{ color: owed <= 0 ? '#5ECDB9' : '#FC6171' }}>
									<b>{numberWithCommas(Math.abs(owed))}</b>
								</div>
							</div>
						</div>
					)
