...
const Grading: React.FC<PropsType> = (props) => {
	//TODO: we need to pull from the student object and see if they have a result already
	const [result, setResult] = useState<MISDiagnosticReport['questions']>(null)

	const { class_name, subject, std_id, section_id, test_id } = props.match.params as Params
	const url = props.match.url.split('/')

	const test = props.targeted_instruction.tests[test_id]
	...
	const handleChange = (val: any, questionId: string) => {
		questionsObj[questionId].is_correct = val
		let res
			...
			{Object.keys(questionsObj)
				...
				.map(function (key, index) {
					return <div key={key} className={`flex flex-row justify-between items-center border border-solid border-gray-200 px-3 ${index % 2 === 0 ? "bg-gray-100" : "bg-white"} h-12`}>
						<span className="text-xs font-bold">{`${key}: `}</span><div className="text-xs w-32 truncate">{questionsObj[key].question_text}</div>
