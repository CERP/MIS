            ...
            <div className="m-3 flex flex-wrap w-full h-20 justify-between">
                {
                    Object.values(students)
                        ...
                        .map((std) => (<Link key={name} className="no-underline h-28 flex flex-col flex items-center justify-center" to={`${(props.location.pathname).substring(0, 36)}/${section_id}/${class_name}/${subject}/${test_id}/insert-grades/${std.id}/grading`}>
                            <img className={std.diagnostic_result[test_id].checked ? "rounded-full h-16 p-1 border-2 border-solid border-green-primary" : "rounded-full h-14 w-14 p-2"} src="https://www.atmeplay.com/images/users/avtar/avtar_nouser.png" alt="img" />
