...
const Subjects: React.FC<PropsType> = (props) => {
    ...
    return <div className="flex flex-wrap flex-row justify-around w-full mx-4">
        {subjects.map((subjectObj) => (
            <div key={subjectObj.subject} className="container w-full sm:px-8 bg-white rounded-lg m-3 h-28 flex items-center justify-start flex-col shadow-lg" onClick={() => props.history.push(`${props.location.pathname}/pdf`)}>
