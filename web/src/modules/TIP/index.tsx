...
const Home: React.FC<PropsType> = ({ faculty, faculty_id }) => {
	...
	return (
		<div className="flex flex-wrap content-between bg-white">
			...
			{
				<Link
					className={`flex justify-center content-center w-full mt-2 no-underline ${layout === Layouts.DIAGNOSTIC && 'opacity-50'
						}`}
					to={'/targeted-instruction/detailed-analysis'}>
					<button className="border border-solid border-blue-900 bg-white text-blue-900 shadow-lg p-2 px-5 rounded-2xl outline-none cursor-pointer">
						View Class / Groups
					</button>
				</Link>
			}
