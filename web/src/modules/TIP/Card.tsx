...
const Card: React.FC<P> = ({ class_name, teacher_name, school_name, subject, lesson_name, lesson_no }) => {
    ...
    const color = class_num === 'KG' ? "light-blue" : class_num === '1' ? "yellow" : class_num === '2' ? "green" : class_num === '3' ? "orange" : "sea-green"
    ...
    return <div className="w-full flex justify-center print:hidden">
        <div className={`${class_name.substring(0, 5) === "Level" ? `bg-${color}-tip-brand` : `bg-sea-green-tip-brand`} container sm:px-8 rounded-md h-20 mb-6 mt-0 w-11/12 md:w-13/12`}>
