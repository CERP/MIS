...
const ChildView: React.FC<PropsType> = ({
	...
	const curr_quizes = targeted_instruction?.quizzes?.[class_name]?.[subject]
			...
			<div className="bg-blue-tip-brand text-white flex flex-row justify-between items-center w-full py-2">
				...
				<div className="w-2/3 flex flex-row justify-start md:mr-5">
					{getQuizTitles()
						.sort(
							(a_quiz_id, b_quiz_id) =>
								curr_quizes?.[b_quiz_id]?.quiz_order -
								curr_quizes?.[a_quiz_id]?.quiz_order
						)
						.slice(page_num * 3, (page_num + 1) * 3)
						.map(quiz_id => {
							const quiz_title = curr_quizes?.[quiz_id]?.quiz_title
							const quiz_order = curr_quizes?.[quiz_id]?.quiz_order
							return (
								<div
									key={quiz_id}
