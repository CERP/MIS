...
class FeeMenu extends Component<propTypes, S> {
	...
	constructor(props: propTypes) {
		...
		const fee_for = parsed_query.forward
			? parsed_query.forward.toString().toUpperCase()
			: 'STUDENT'
				...
				<div className="reports-menu">
					...
					<div className="sub-list" style={{ width: '100%' }}>
						{this.state.fee_for === 'STUDENT' ? (
							<StudentList {...this.props} forwardTo="payment" />
						) : (
							<ClassListModule {...this.props} forwardTo="fee-menu" />
						)}
					</div>
				</div>
			</Layout>
