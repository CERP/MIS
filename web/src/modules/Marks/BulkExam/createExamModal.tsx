...
const CreateExamModal: React.FC<PropsType> = ({ onCreate, onClose, subjects }) => {
	...
	return (
		<div className="bulk-exams modal-container inner">
			...
			<div className="section-container section">
				<div className="space-y-2">
					{stateProps.isExamCreated && (
						<div className="text-center bold" style={{ color: '#5ecdb9' }}>
					...
					<div className="flex flex-col md:flex-row justify-between">
						...
						<select
							className="tw-select w-full md:w-1/2"
							onChange={e =>
								setStateProps({ ...stateProps, subject: e.target.value })
							}
							value={stateProps.subject}>
					...
					<div className="flex flex-col md:flex-row justify-between">
						...
						<input
							className="tw-input w-full md:w-1/2"
							type="number"
							value={stateProps.totalScore}
							onChange={e =>
								setStateProps({ ...stateProps, totalScore: e.target.value })
							}
					...
					<div className="flex flex-col md:flex-row justify-between">
						...
						<input
							...
							onChange={e =>
								setStateProps({
									...stateProps,
									date: moment(e.target.value, 'YYYY-MM-DD').unix() * 1000
								})
							}
							defaultValue={moment(current_date).format('YYYY-MM-DD')}
							placeholder="Enter total marks"
						/>
					</div>
				</div>
				<div className="row" style={{ marginTop: 15, justifyContent: 'flex-end' }}>
					<div
						className={`button blue ${isDisabled() ? 'disabled' : ''}`}
						onClick={createExam}>
						Create Exam
					</div>
				</div>
