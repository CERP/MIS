...
const deriveSelectedStudents = (selected_section: string, students: RootDBState['students']) =>
	getStudentsForSection(selected_section, students).reduce(
...
class Attendance extends Component<propTypes, S> {
	...
	constructor(props: propTypes) {
		...
		const selected_section =
			my_sections.length > 0 ? my_sections[0].id : sections.length > 0 ? sections[0].id : ''
	...
	selectAllOrNone = () => {
		...
		if (all_selected) {
			// set all to false
			this.setState({
				selected_students: Object.keys(this.state.selected_students).reduce(
					(agg, curr) => ({ ...agg, [curr]: false }),
					{}
				),
			})
		} else {
	...
	selectPresentOrNone = () => {
		...
		const selection = Object.keys(this.state.selected_students).reduce((agg, sid) => {
			const x = students[sid]

			const current_attendance = (x.attendance || {})[
	...
	relevant_students = () => {
		Object.keys(this.state.selected_students).map((id) => this.props.students[id])
	}

	getLeaveStatus = (s: MISStudentAttendanceEntry['status'] | 'n/a'): boolean =>
		s === 'LEAVE' || s === 'SHORT_LEAVE' || s === 'SICK_LEAVE' || s === 'CASUAL_LEAVE'
	...
	onQrScan = (potential_id: string) => {
		...
		if (this.props.students[potential_id]) {
			this.props.markStudent(
				this.props.students[potential_id],
	...
	render() {
		...
		const messages = Object.entries(this.state.selected_students).reduce(
			(agg, [sid, selected]) => {
				if (!selected) {
					return agg
				}
					...
					<Modal>
						<div className="scan-modal-div modal-container inner">
							<div
								...
								onClick={() => this.setState({ qr_mode: false, student_name: '' })}>
								X
							</div>
							<QrReader
								delay={300}
							...
							<div className="row">
								{this.state.student_name && (
									<div className="stdName">
										Student Name : {this.state.student_name}
									</div>
								)}
							</div>
						</div>
				...
				<div className="attendance">
					...
					<div className="row" style={{ justifyContent: 'space-between', width: '90%' }}>
						...
						<div
							...
							onClick={() => this.setState({ qr_mode: !this.state.qr_mode })}>
							Scan ID Card
						</div>
					</div>
					...
					<div className="selectors">
						<div className="row" style={{ flexWrap: 'wrap' }}>
							<div className="button select-all" onClick={this.selectAllOrNone}>
								{Object.values(this.state.selected_students).every((x) => x)
									? 'Select None'
									: 'Select All'}
							</div>
							<div className="button select-all" onClick={this.selectPresentOrNone}>
								P : {attendanceStats.present}
							</div>
							<div className="button select-all" onClick={this.selectAbsentOrNone}>
								A : {attendanceStats.absent}
							</div>
							<div className="button select-all" onClick={this.selectLeaveOrNone}>
								L : {attendanceStats.leave}
							</div>
							<div className="button" onClick={this.markAllPresent}>
								Mark All Present
							</div>
						</div>
					...
					<div className="list">
						{Object.keys(this.state.selected_students)
							...
							.map((sid) => {
								...
								const status = current_attendance
					...
					{Object.values(this.state.selected_students).some((x) => x) ? (
						<a
							href={url}
							className="button blue"
							onClick={() => this.sendBatchSMS(messages)}>
							Send SMS
						</a>
					) : (
						false
...
export default connect(
	(state: RootReducerState) => ({
		...
		attendance_message_template:
			(state.db.sms_templates || ({} as RootDBState['sms_templates'])).attendance || '',
	}),
	(dispatch: Function) => ({
		markAllStudents: (
			students: MISStudent[],
			date: string,
			status: MISStudentAttendanceEntry['status']
		) => dispatch(markAllStudents(students, date, status)),
		markStudent: (
			student: MISStudent,
			date: string,
			status: MISStudentAttendanceEntry['status']
		) => dispatch(markStudent(student, date, status)),
