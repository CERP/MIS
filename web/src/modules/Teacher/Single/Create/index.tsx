...
class CreateTeacher extends Component<propTypes, S> {
	...
	constructor(props: propTypes) {
		...
		this.state = {
			profile: {
				...
				permissions:
					this.isFirst() || this.isNew() || !faculty.permissions
	...
	onSave = () => {
		...
		const compulsoryFileds = checkCompulsoryFields(profile, [['Name'], ['Password']])
	...
	render() {
		...
		return (
			<div className="single-teacher-create">
				...
				{this.isNew() && !this.state.toggleMoreInfo && (
					<div className="form">
						...
						<div className="row">
							...
							<input
								type="password"
								{...this.former.super_handle_flex(['Password'], {
									styles: (val: string) => {
										return val === '' ? { borderColor: '#fc6171' } : {}
									}
								})}
				...
				{(this.isNew() ? this.state.toggleMoreInfo : true) && (
					<div className="form">
						<div className="divider">Personal Information</div>
						<div className="row">
							<label>Full Name</label>
						...
						{!this.isFirst() && (
							<>
								...
								<div className="row">
									<label>Gender</label>
									<select
										{...this.former.super_handle(['Gender'])}
										disabled={!canEdit}>
								...
								<div className="row">
									<label>Married</label>
									<select
										{...this.former.super_handle(['Married'])}
										disabled={!canEdit}>
										<option value="" disabled>
								...
								<div className="row">
									...
									<input
										...
										placeholder="Father/Husband CNIC"
										disabled={!canEdit}
									/>
								</div>
								...
								<div className="row">
									<label>Admin Status</label>
									<select
										{...this.former.super_handle(['Admin'])}
										disabled={!admin}>
										<option value="true">Admin</option>
										<option value="false">Not an Admin</option>
									</select>
								...
								<div className="row">
									...
									<select
										{...this.former.super_handle(['HasLogin'])}
										disabled={!admin}>
						...
						{!this.isFirst() && (
							<>
								...
								<div className="row" style={{ flexDirection: 'row' }}>
									...
									/>
									<datalist id="tags">
										{this.getUniqueTagsFromFaculty().map(tag => (
								...
								<div className="row">
									...
									<select
										{...this.former.super_handle(['StructuredQualification'])}
										disabled={!admin}>
										<option value="" disabled>
								...
								<div className="row">
									...
									<textarea
										{...this.former.super_handle(['Qualification'])}
								...
								<div className="row">
									...
									<textarea
										{...this.former.super_handle(['Experience'])}
								...
								<div className="row">
									...
									<input
										type="number"
										{...this.former.super_handle(['Salary'])}
								...
								<div className="row">
									...
									<input
										type="date"
										onChange={this.former.handle(['HireDate'])}
										value={moment(this.state.profile.HireDate).format(
											'YYYY-MM-DD'
										)}
								...
								<div className="row">
									...
									<select
										{...this.former.super_handle(['Active'])}
										disabled={!admin}>
										<option value="" disabled>
				...
				{!admin ? (
					...
					<div className="section-container" style={{ marginTop: '0.5rem' }}>
						...
						{this.isNew() || this.isFirst() ? (
							false
						) : (
							<div className="button red" onClick={this.onDelete}>
								Delete
							</div>
						)}
					</div>
...
export default connect(
	...
	(dispatch: Function) => ({
		save: (teacher: MISTeacher, is_first?: boolean) =>
			dispatch(createFacultyMerge(teacher, is_first)),
