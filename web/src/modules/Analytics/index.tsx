				...
				<div className="analytics">
					<div className="row tabs">
						{!this.props.ilmxUser && (
							<>
								<Link
									className={`button ${loc === 'fees' ? 'orange' : ''}`}
									to="fees"
									replace={true}>
									Fees
								</Link>
								...
								<Link
									className={`button ${loc === 'ilmexchange' ? 'blue' : ''}`}
									to="ilmexchange"
									replace={true}>
									Ilm Exchange
								</Link>
...
export default connect((state: RootReducerState) => ({ db: state.db, ilmxUser: getIlmxUser() }))(
	Analytics
)
