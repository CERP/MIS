			...
			<LineChart
				data={Object.entries(attendance)
					...
					.map(
						([
							month,
							{ PRESENT, LEAVE, ABSENT, CASUAL_LEAVE, SHORT_LEAVE, SICK_LEAVE }
						]) => ({
							month,
							PRESENT,
							LEAVE: LEAVE + CASUAL_LEAVE + SHORT_LEAVE + SICK_LEAVE,
							ABSENT,
							percent: (
								(ABSENT /
									(ABSENT +
				...
				{filter.present && (
					<Line dataKey="PRESENT" stroke="#93d0c5" strokeWidth={3} name="Present" />
				)}
				{filter.absent && (
					<Line dataKey="ABSENT" stroke="#ff6b68" strokeWidth={3} name="Absent" />
				)}
				{filter.leave && (
					<Line dataKey="LEAVE" stroke="#807f7f" strokeWidth={3} name="Leave" />
				)}
				{filter.percentage && (
					<Line dataKey="percent" stroke="#74aced" strokeWidth={3} name="Percentage" />
				)}
			</LineChart>
		</ResponsiveContainer>
	)
...
const AttendanceTable = ({ attendance, totals, date_format }: TableData) => {
	return (
		<div
			className="section table line w-full"
			style={{ margin: '20px 0', backgroundColor: '#c2bbbb21' }}>
			<div className="mis-table row heading">
				<label style={{ backgroundColor: '#efecec' }}>
					<b>Date</b>
				</label>
				<label style={{ backgroundColor: '#93d0c5' }}>
					<b>Present</b>
				</label>
				<label style={{ backgroundColor: '#fc6171' }}>
					<b>Absent</b>
				</label>
				<label style={{ backgroundColor: '#e0e0e0' }}>
					<b>Leave</b>
				</label>
				<label style={{ backgroundColor: '#bedcff' }}>
					<b>Absentee(%)</b>
				</label>
			</div>
			{[
						...
						([
							...
							<div className="mis-table row" key={month}>
								...
								<div style={{ backgroundColor: '#e0e0e0' }}>
									{LEAVE + CASUAL_LEAVE + SHORT_LEAVE + SICK_LEAVE}
								</div>
								<div style={{ backgroundColor: '#bedcff' }}>
									{(
										(ABSENT /
											(ABSENT +
												PRESENT +
	...
	calculate = () => {
		...
		const reducify = () => {
			...
			for (const [date, record] of Object.entries(student.attendance)) {
				if (
					!(
						moment(date).isSameOrAfter(temp_sd, 'day') &&
						moment(date).isSameOrBefore(temp_ed, 'day')
					)
				) {
					continue
				}
				...
				const m_status = attendance[period_key] || {
					PRESENT: 0,
					LEAVE: 0,
					ABSENT: 0,
					SHORT_LEAVE: 0,
			...
			<div className="attendance-analytics mx-auto">
				...
				<div className="no-print btn-filter-toggle row">
					<button
						...
						onClick={() =>
							this.setState({
								isStudentAttendanceFilter: !this.state.isStudentAttendanceFilter
							})
						}>
						Show Filters
					</button>
				</div>
				{this.state.isStudentAttendanceFilter && (
				...
				<div className="section">
					<div className="row no-print space-x-2">
						<input
