...
const ExpenseChart: React.SFC<ChartProps> = ({ collective_obj, chartFilter, date_format }) => {
	return (
		<ResponsiveContainer width="100%" height={200}>
			<LineChart
				data={Object.entries(collective_obj)
				...
				{chartFilter.income && (
					<Line dataKey="income" name="Income" stroke="#bedcff" strokeWidth={3} />
				)}
				{chartFilter.expense && (
					<Line dataKey="expense" stroke="#e0e0e0" name="Expense" strokeWidth={3} />
				)}
				{chartFilter.profit && (
					<Line dataKey="profit" stroke="#93d0c5" name="Profit" strokeWidth={3} />
				)}
			</LineChart>
		</ResponsiveContainer>
	)
...
const ExpenseTable: React.SFC<TableProps> = ({
	collective_obj,
	total_income,
	total_expense,
	date_format,
	...
	return (
		<div className="section mistable">
					...
					.map(([month, { income, expense }]) => {
						return (
							<div className="mis-table row" key={month}>
								...
								<div style={{ backgroundColor: '#93d0c5' }}>
			...
			<div className="mis-table row footing">
				...
				<label style={{ backgroundColor: '#bedcff' }}>
					<b>{numberWithCommas(total_income)}</b>
				</label>
				<label style={{ backgroundColor: '#e0e0e0' }}>
					<b>{numberWithCommas(total_expense)}</b>
				</label>
				<label style={{ backgroundColor: '#93d0c5' }}>
					<b>{numberWithCommas(Math.round(total_income - total_expense))}</b>
				</label>
			</div>
		</div>
	)
