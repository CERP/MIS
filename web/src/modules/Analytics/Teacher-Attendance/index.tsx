...
const AttendanceTable = ({ attendance, totals, date_format }: TableData) => {
	return (
		<div
			className="section mis-table line"
			...
			{[
				...Object.entries(attendance)
					.sort(([d1], [d2]) => moment(d1, date_format).diff(moment(d2, date_format)))
					.map(([month, { PRESENT, LEAVE, ABSENT }]) => (
						<div className="mis-table row" key={Math.random()}>
							...
							<div style={{ backgroundColor: '#bedcff' }}>
								{(ABSENT / (ABSENT + PRESENT + LEAVE)) * 100
									? ((ABSENT / (ABSENT + PRESENT + LEAVE)) * 100).toFixed(2)
									: '0'}
								%
							</div>
						</div>
				...
				<div
					...
					<label style={{ backgroundColor: '#bedcff' }}>
						<b>
							{(
								(totals.ABSENT / (totals.ABSENT + totals.PRESENT + totals.LEAVE)) *
								100
							).toFixed(2)}
							%
						</b>
	...
	render() {
		...
		for (const [tid, teacher] of Object.entries(teachers)) {
			...
			for (const [date, record] of Object.entries(teacher.attendance)) {
				if (
					!(moment(date).isSameOrAfter(temp_sd) && moment(date).isSameOrBefore(temp_ed))
				) {
					continue
				}
		...
		const items = Object.entries(teacher_attendance).sort(
			([, { ABSENT: a1 }], [, { ABSENT: a2 }]) => a2 - a1
		)
				...
				<div className="section">
					...
					{items
						...
						.map(([tid, { teacher, PRESENT, ABSENT, LEAVE }]) => {
							return (
								<div key={tid} className="mis-table row">
