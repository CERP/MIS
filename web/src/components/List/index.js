...
class List extends Component {
	...
	render() {
		...
		const filteredList = items
			...
			.sort(
				(a, b) =>
					toLabel(b).localeCompare(this.state.filterText) -
		...
		return (
			<div className="list-wrap">
				<div className="total">
					<div className="label">
						Total:{' '}
						<strong> {header ? filteredList.length - 1 : filteredList.length} </strong>
					</div>
					{this.props.create ? (
						<Create to={this.props.create} text={this.props.createText} />
					) : (
						false
					)}
				</div>
