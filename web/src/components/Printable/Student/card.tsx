			...
			<div className="card-row first">
				...
				<div className="card-student-info">
					<div style={{ fontSize: '.75rem' }}>
						<div>
							Name: <span>{toTitleCase(student.Name)}</span>
						</div>
						<div>
							Class: <span className="name-wrap">{studentClass}</span>
						</div>
						<div>
							Roll No: <span>{student.RollNumber}</span>
						</div>
						<div>
							Valid for:{' '}
							<span>
								{schoolSession.startYear}-{schoolSession.endYear}
							</span>
						</div>
					</div>
