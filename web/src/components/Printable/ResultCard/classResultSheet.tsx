...
export const ClassResultSheet = (props: PropsTypes) => {
	...
	const formatMarks = (marks_obtained: number): string => {
		...
		if (marks_obtained === 0) return ''

		return Number.isInteger(marks_obtained)
			? marks_obtained.toString()
			: marks_obtained.toFixed(2)
	}
		...
		<div className="print-table print-page">
			<table className="outer-space">
				<caption>
					...
					<div>
						Class: {props.sectionName} | Exam: {props.examName} -{' '}
						{moment().format('YYYY')}
					</div>
				</caption>
				<thead>
					<tr>
						<th className="result-sheet" style={{ width: '4%' }}>
							R No.
						</th>
						<th className="result-sheet" style={{ width: '15%' }}>
							Name
						</th>
				...
				<tbody>
					{props.students.map((student: StudentMarksSheet, index) => {
						return (
								...
								{props.relevant_exams
									...
									.map(exam => {
										...
										if (aug_exam === undefined) {
											return (
												<td key={exam.id} className="cell-center">
													-
												</td>
											)
										}
										...
										return (
											<td key={aug_exam.id} className="cell-center">
												{' '}
												{aug_exam.stats.score}{' '}
											</td>
										)
									})}
								<td className="cell-center">{`${formatMarks(
									student.marks.obtained
								)}/${student.marks.total}`}</td>
