...
const ResultCard = (props: PropsType) => {
	...
	const result_card_title =
		exam_title === 'Test' && month !== '' ? `${exam_title}-${month}` : exam_title
		...
		for (const [date, record] of Object.entries(attendance)) {
			if (
				moment(date).isBetween(
					moment(schoolSession.start_date),
					moment(schoolSession.end_date)
				)
			) {
	...
	const total_leave_days =
		attendance.LEAVE + attendance.SHORT_LEAVE + attendance.SICK_LEAVE + attendance.CASUAL_LEAVE
	...
	return (
		<div className="school-result-card">
			...
			<div className="result-card-header">
				<div className="row">
					<div className="school-session">
						{' '}
						Session: {moment(schoolSession.start_date).format('YYYY')} -{' '}
						{moment(schoolSession.end_date).format('YYYY')}{' '}
					</div>
				...
				<div className="student-info">
					...
					<div className="row">
						...
						<div className="bold text-underline">
							{student.ManName ? student.ManName : ''}
						</div>
					</div>
					<div className="row">
					...
					<div className="row">
						...
						<div className="bold">
							{formatNumber(attendance_percentage)}% (P: {attendance.PRESENT}, L:{' '}
							{total_leave_days}, A: {attendance.ABSENT})
						</div>
					</div>
				</div>
			...
			<div className="print-table" style={{ height: '100mm' }}>
				<table>
					...
					<tbody>
						{student.merge_exams.map((exam, i) => (
							<tr key={exam.id}>
								<td className="text-center">
									{listBy === 'Date' ? moment(exam.date).format('MM/DD') : i + 1}
								</td>
								<td>{exam.subject}</td>
								<td className="text-center">{exam.total_score}</td>
								<td className="text-center">
									{exam.stats.grade !== 'Absent' ? exam.stats.score : 'N/A'}
								</td>
								<td className="text-center">
									{exam.stats.grade !== 'Absent'
										? formatNumber((exam.stats.score / exam.total_score) * 100)
										: 'N/A'}
								</td>
					...
					<tfoot>
						<tr className="bold" style={{ height: 0.8 }}>
							...
							<td className="text-center">
								{formatNumber((marks.obtained / marks.total) * 100)}%
							</td>
							<td colSpan={2}></td>
						</tr>
					</tfoot>
				...
				<div className="result-stats">
					<div className="row">
						Grade: &nbsp; <b>{calculateGrade(marks.obtained, marks.total, grades)}</b>
					</div>
