...
const getDefaultFeeForClasses = (classes: RootDBState['classes']): State => {
	return Object.keys(classes).reduce((agg, curr) => {
		return {
			...agg,
			[curr]: {
				name: 'Monthly',
	...
	return (
		<div className="my-4 p-5 space-y-4 w-full md:w-9/12 mx-auto">
								...
								<div key={c.id} className="table-row bg-gray-100">
									...
									<div className="table-cell p-2">
										<input
											...
											type="number"
											value={
												(state[c.id].amount as unknown) === 0
													? ''
													: state[c.id].amount
											}
											className="w-20 md:w-28 py-1 tw-input"
										/>
									</div>
								</div>
							))}
			...
			<div className="flex justify-end">
				<button onClick={saveDefaultFee} className="tw-btn-blue w-full md:w-1/4">
					Save Fee Settings
				</button>
			</div>
		</div>
