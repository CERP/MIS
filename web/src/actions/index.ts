...
export const createFacultyMerge =
	(faculty: MISTeacher, is_first?: boolean) => (dispatch: Function) => {
		dispatch(
			createMerges([
				{
...
export const uploadFacultyProfilePicture =
	(facultyId: string, imageString: string) => (dispatch: Function) => {
...
export const deleteFaculty =
	(faculty_id: string) => (dispatch: Function, getState: () => RootReducerState) => {
...
export const promoteStudents =
	(promotion_map: PromotionMap, section_metadata: Section[]) => (dispatch: Function) => {
		...
		const merges = Object.entries(promotion_map).reduce(
			(agg, [student_id, { current, next }]) => {
...
export const uploadStudentProfilePicture =
	(student: MISStudent, image_string: string) =>
	(dispatch: Function, getState: () => RootReducerState, syncr: Syncr) => {
...
export const createSchoolLogin =
	(school_id: string, password: string) =>
	(dispatch: Function, getState: () => RootReducerState, syncr: Syncr) => {
		const action = {
			type: SCHOOL_LOGIN,
			school_id,
...
export const autoSchoolLogin =
	(school_id: string, token: string, client_id: string, refcode: string) =>
	(dispatch: Function, getState: () => RootReducerState, syncr: Syncr) => {
		const action = {
			type: SCHOOL_LOGIN
		}
...
export const deleteClass =
	(Class: MISClass) => (dispatch: Function, getState: () => RootReducerState) => {
		const state = getState()
...
export const deleteSection =
	(classId: string, sectiondId: string) =>
	(dispatch: Function, getState: () => RootReducerState) => {
		const state = getState()
...
export const addStudentToSection =
	(section_id: string, student: MISStudent) => (dispatch: Function) => {
		dispatch(
			createMerges([
				{
					path: ['db', 'students', student.id, 'section_id'],
...
export const addStudentToFamily =
	(student: MISStudent, family_id: string) => (dispatch: Function) => {
		dispatch(
			createMerges([
				{
...
export const saveFamilyInfo =
	(siblings: MISStudent[], info: MISFamilyInfo, famId?: string) => (dispatch: Function) => {
...
export const markFaculty =
	(faculty: MISTeacher, date: string, status: MISTeacherAttendanceStatus, time = moment.now()) =>
	(dispatch: Function) => {
...
export const undoFacultyAttendance =
	(faculty: MISTeacher, date: string) => (dispatch: Function) => {
		dispatch(
			createDeletes([
				{
...
export const assignLearningLevel =
	(student_id: string, subject: TIPSubjects, level: TIPGrades) => (dispatch: Function) => {
		dispatch(
			createMerges([
				{
					path: [
...
export const mergeTIPResult =
	(student_id: string, diagnostic_report: TIPDiagnosticReport, test_id: string) =>
	(dispatch: Function) => {
		dispatch(
			createMerges([
				{
...
export const resetStudentLearningLevel =
	(student_id: string, subject: TIPSubjects) => (dispatch: Function) => {
		dispatch(
			createDeletes([
				{
					path: [
...
export const addHistoricalPayment =
	(payment: historicalPayment, student_id: string) => (dispatch: Function) => {
...
export const editExpense =
	(expenses: ExpenseEditItem) => (dispatch: Function, getState: () => RootReducerState) => {
...
export const createTemplateMerges =
	(templates: RootDBState['sms_templates']) => (dispatch: Function) => {
		dispatch(
			createMerges([
				{
...
export const mergeExam =
	(exam: Exam, class_id: string, section_id: string) => (dispatch: Function) => {
		// exam is
...
export const addDiary =
	(date: string, section_id: string, diary: MISDiary['section_id']) => (dispatch: Function) => {
...
export const issueCertificate =
	(type: string, student_id: string, faculty_id: string) => (dispatch: Function) => {
...
export const saveDateSheet =
	(datesheetMerges: DateSheetMerges, section_id: string) => (dispatch: Function) => {
...
export const removeSubjectFromDatesheet =
	(id: string, subj: string, section_id: string) => (dispatch: Function) => {
		dispatch(
			createDeletes([
				{
					path: ['db', 'planner', 'datesheet', section_id, id, subj]
...
export const sendTempPassword =
	(faculty: MISTeacher, password: string) =>
	(dispatch: Function, getState: () => RootReducerState, syncr: Syncr) => {
		if (!syncr.ready) {
