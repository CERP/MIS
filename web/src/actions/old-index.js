...
export const createEditClass = (newClass) => (dispatch) => {
	dispatch(createMerges([{ path: ['db', 'classes', newClass.id], value: newClass }]))
}
...
export const deleteClass = (Class) => (dispatch, getState) => {
	...
	const students = Object.values(state.db.students)
		...
		.map((student) => ({ path: ['db', 'students', student.id, 'section_id'], value: '' }))
...
export const addStudentToSection = (section_id, student) => (dispatch) => {
	dispatch(
		createMerges([{ path: ['db', 'students', student.id, 'section_id'], value: section_id }])
	)
}
...
export const addExpense = (amount, label, type, category, quantity, date, time = moment.now()) => (
	dispatch
) => {
