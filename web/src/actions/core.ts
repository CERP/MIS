...
const getRationalizedQueuePayload = (
	payload: any,
	key: keyof RootReducerState['queued'],
	state: RootReducerState
): RootReducerState['queued'] => {
...
export const analyticsEvent = (event: BaseAnalyticsEvent[]) => (
	dispatch: Function,
	...
	const rationalized_event_payload = getRationalizedQueuePayload(
		event_payload,
		'analytics',
		state
	)
...
export const uploadImages = (images: ImageMergeItem[]) => (
	dispatch: (a: any) => any,
	getState: () => RootReducerState,
	syncr: Syncr
) => {
...
export const processImageQueue = () => (
	dispatch: (a: any) => any,
	getState: () => RootReducerState,
	syncr: Syncr
) => {
	...
	const items = Object.entries(state.queued.images || {}).filter(
		([k, v]) => v.status === 'queued'
	)
...
const markImagesInQueue = (images: ImagesQueuable, status: QueueStatus) => (
	dispatch: (a: any) => any
) => {
	const mapped = Object.entries(images).reduce<ImagesQueuable>((agg, [k, v]) => {
		return {
			...agg,
			[k]: {
				...v,
...
const Sync = (payload: RootReducerState['queued']) => (
	dispatch: (a: any) => any,
	getState: () => RootReducerState,
	syncr: Syncr
) => {
